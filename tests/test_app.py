...
def test_sunday_and_holidays_off_and_rules(client):
    ...
    u_koba = db.query(models.User).filter(models.User.full_name == "小林").first()
    ...
    u_ieda = db.query(models.User).filter(models.User.full_name == "家田").first()
